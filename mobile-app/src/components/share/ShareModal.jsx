    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[99999] flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="share-modal-title">
      <div className="bg-dark-surface border border-primary/20 p-6 rounded-2xl max-w-sm w-full space-y-5 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex flex-col items-center justify-center bg-dark-bg p-4 rounded-xl border border-dark-border">
          <img src={qrUrl} alt="QR Code" className="w-[200px] h-[200px] rounded-lg shadow-md bg-dark-surface p-2 border border-primary/30" />
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2">
            <input type="text" readOnly value={shareUrl || ''} className="w-full bg-dark-bg border border-dark-border rounded-lg px-3 py-2 text-xs text-dark-text font-mono truncate focus:outline-none focus:border-primary" />
            <button onClick={handleCopy} className="bg-primary/10 hover:bg-primary/20 text-primary border border-primary/30 px-3 py-2 rounded-lg text-xs font-medium flex items-center gap-1.5 transition-colors cursor-pointer shrink-0">
        ...
        <div className="grid grid-cols-2 gap-2 pt-1">
          <a href={waUrl} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 py-2 rounded-xl text-xs font-medium transition-colors">
            <MessageCircle size={16} />
            <span>WhatsApp</span>
          </a>
          <a href={tgUrl} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 bg-sky-500/10 hover:bg-sky-500/20 text-sky-400 border border-sky-500/30 py-2 rounded-xl text-xs font-medium transition-colors">
            <Send size={16} />
            <span>Telegram</span>
          </a>
        </div>
