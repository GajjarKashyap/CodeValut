-- ============================================================
-- CODEVAULT — ADMIN SINGLE DEVICE ENFORCEMENT
-- Admin can only be logged in on ONE device at a time.
-- When admin logs in on a new device, all old sessions are revoked.
...
BEGIN
  ...
  -- ============================================================
  -- ADMIN: single-device enforcement
  -- Revoke all other sessions, approve this device exclusively
  -- ============================================================
  IF v_email = 'admin@admin.com' THEN
    -- Revoke ALL existing active sessions (kick admin off old devices)
    UPDATE public.auth_device_sessions
    SET status = 'revoked', revocation_reason = 'Admin logged in on a new device'
