-- ============================================================
-- CODEVAULT — DIRECT GRANT & LOGIN POLICY FIX
...
BEGIN
  ...
  ELSIF v_active_sessions_count = 0 THEN
    ...
    UPDATE public.user_devices SET status = 'approved', is_primary = true, approved_at = now() WHERE id = v_device.id;
    v_status := 'active';
  ELSIF v_settings.login_policy = 'allow_all' AND v_active_sessions_count < v_settings.max_active_devices THEN
    -- Direct Grant (allow_all): auto-approve if under device limit
