-- ============================================================
-- CODEVAULT — ADMIN TOGGLE: SINGLE DEVICE MODE FOR USERS
-- Adds single_device_mode column and updates login logic.
-- Paste into: Supabase Dashboard → SQL Editor → Run
-- ============================================================

-- Step 1: Add single_device_mode boolean to user_login_settings
ALTER TABLE public.user_login_settings 
ADD COLUMN IF NOT EXISTS single_device_mode boolean DEFAULT false;

-- Step 2: Update the registration function to respect single_device_mode
...
BEGIN
  ...
  INSERT INTO public.user_login_settings (user_id) VALUES (p_user_id)
  ...
  WHERE user_id = p_user_id AND device_id = v_device.id AND status IN ('active', 'pending')
  ...
  IF v_existing_session IS NOT NULL AND v_existing_session.status = 'pending' THEN
    SELECT * INTO v_existing_request FROM public.device_login_requests
    WHERE device_session_id = v_existing_session.id AND status = 'pending' AND expires_at > now()
    LIMIT 1;
    IF v_existing_request IS NOT NULL THEN
      RETURN jsonb_build_object(
        'status','pending','device_id',v_device.id,
    ...
    END IF;
    UPDATE public.auth_device_sessions SET status='revoked', revocation_reason='Approval expired'
    WHERE id = v_existing_session.id;
  END IF;

  -- ============================================================
  -- SINGLE DEVICE MODE: kicks off old devices automatically
  -- Applies to admin naturally, OR if single_device_mode is enabled
  -- ============================================================
  IF v_email = 'admin@admin.com' OR v_settings.single_device_mode = true THEN
    -- Revoke ALL existing active sessions
    UPDATE public.auth_device_sessions
    SET status = 'revoked', revocation_reason = 'Account logged in on a new device (Single Device Mode)'
    WHERE user_id = p_user_id AND status = 'active';

    -- Revoke all other devices
    UPDATE public.user_devices
    SET status = 'revoked', revoked_at = now()
    WHERE user_id = p_user_id AND id != v_device.id AND status = 'approved';

    -- Approve this device as primary
    UPDATE public.user_devices
  ...
  ELSIF v_device.status = 'approved' THEN
    SELECT count(*) INTO v_active_count FROM public.auth_device_sessions
    WHERE user_id = p_user_id AND status = 'active';
    IF v_device.is_primary OR v_active_count < v_settings.max_active_devices THEN
      v_status := 'active';
    ELSE
      RETURN jsonb_build_object('status','limit_reached','reason','Maximum active device limit reached. Go to Settings to increase it.');
    END IF;

  ELSIF NOT EXISTS (SELECT 1 FROM public.auth_device_sessions WHERE user_id = p_user_id AND status = 'active') THEN
  ...
  ELSIF v_settings.login_policy = 'allow_all' THEN
    SELECT count(*) INTO v_active_count FROM public.auth_device_sessions WHERE user_id=p_user_id AND status='active';
    IF v_active_count < v_settings.max_active_devices THEN
      UPDATE public.user_devices SET status='approved', approved_at=now() WHERE id=v_device.id;
      v_status := 'active';
    ELSE
  ...
  IF v_status = 'pending' THEN
    ...
    BEGIN
      ...
      VALUES (p_user_id,v_device.id,v_new_session_id,v_code,now()+interval '5 minutes')
      RETURNING id INTO v_req_id;
      RETURN jsonb_build_object(
        'status','pending','device_id',v_device.id,
        'device_session_id',v_new_session_id,
        'request_id',v_req_id,'approval_code_hash',v_code
      );
    END;
  END IF;
